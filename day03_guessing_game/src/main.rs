use rand::Rng;
use std::cmp::Ordering;
use std::fs;
use std::io;
use std::io::Write;

fn write_high_score(high_score: i32) {
    let system_score = fs::read_to_string("highscore").unwrap_or_else(|_| "0".to_string());
    if system_score.trim().parse::<i32>().unwrap_or(0) <= high_score {
        fs::write("highscore", high_score.to_string()).expect("Error writing to file");
    };
}

fn read_high_score() -> i32 {
    let score = match fs::read_to_string("highscore") {
        Ok(n) => n,
        Err(_e) => {
            let mut file = fs::File::create("highscore").expect("Error creating file");
            file.write_all(b"0").expect("Error writing default score");
            "0".to_string()
        }
    };
    score.trim().parse().unwrap_or(0)
}

fn play() {
    let mut high_score = 0;
    let mut secret_number = rand::thread_rng().gen_range(1..=100);
    println!("Guess the number!");

    let mut no_tries = 3;
    loop {
        if no_tries > 0 {
            println!("Please input your guess.");
            println!("You have {no_tries} guess(es) left");

            let mut guess = String::new();
            io::stdin()
                .read_line(&mut guess)
                .expect("Failed to read line");

            let guess: u32 = match guess.trim().parse() {
                Ok(num) => num,
                Err(_) => {
                    println!("Invalid input! Please enter a valid number.");
                    continue;
                }
            };

            println!("You guessed: {}", guess);

            match guess.cmp(&secret_number) {
                Ordering::Less => println!("Too small!"),
                Ordering::Greater => println!("Too big!"),
                Ordering::Equal => {
                    println!("You win!");
                    high_score += 1;
                    println!("Your score is {high_score}");

                    if !ask_play_again() {
                        write_high_score(high_score);
                        println!("Thanks for playing");
                        break;
                    }

                    secret_number = rand::thread_rng().gen_range(1..=100);
                    no_tries = 3;
                }
            }
        } else {
            println!("You have used up your guesses.");

            if !ask_play_again() {
                write_high_score(high_score);
                println!("Thanks for playing");
                break;
            }

            secret_number = rand::thread_rng().gen_range(1..=100);
            no_tries = 3;
        }

        no_tries -= 1;
    }
}

fn ask_play_again() -> bool {
    println!("Do you want to play again (y/n)");
    let mut play_again = String::new();

    io::stdin()
        .read_line(&mut play_again)
        .expect("Failed to read your choice");

    play_again.trim().starts_with('y')
}

fn main() {
    println!("Welcome to my guess the number game");

    loop {
        println!("What would you like to do?");
        println!("1. Play a new game");
        println!("2. View highscore");
        println!("3. Exit");

        let mut user_selection = String::new();
        io::stdin()
            .read_line(&mut user_selection)
            .expect("Error reading your input");

        match user_selection.trim().parse::<i32>() {
            Ok(1) => play(),
            Ok(2) => {
                let high_score = read_high_score();
                println!("Your highscore is: {high_score}");
            }
            Ok(3) => {
                println!("Thanks for playing!");
                break;
            }
            _ => println!("Please enter a valid option (1, 2, or 3)"),
        }
    }
}
